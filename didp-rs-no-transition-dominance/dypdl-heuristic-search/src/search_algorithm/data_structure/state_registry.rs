...
impl<K> From<StateInRegistry<K>> for dypdl::State
where
    K: Hash
        + Eq
        + Clone
...
{
    fn from(state: StateInRegistry<K>) -> dypdl::State {
        dypdl::State {
            signature_variables: (*state.signature_variables).clone().into(),
...
mod tests {
    ...
    #[test]
    fn state_from_state_in_registry() {
        let mut set1 = Set::with_capacity(3);
        set1.insert(0);
        set1.insert(2);
        let mut set2 = Set::with_capacity(3);
        ...
        let state = dypdl::State::from(StateInRegistry {
    ...
    fn insert_get_new_information() {
        ...
        let (information1, dominated) = information.unwrap();
        ...
        let information = registry.insert(information);
        assert!(information.is_some());
        let (information2, dominated) = information.unwrap();
        assert_eq!(dominated, None);
        ...
        let information = registry.insert(information);
        assert!(information.is_some());
        let (information3, dominated) = information.unwrap();
        assert_eq!(dominated, None);
        ...
        let information = registry.insert(information);
        assert!(information.is_some());
        let (information4, dominated) = information.unwrap();
        assert_eq!(dominated, None);
    ...
    fn insert_get_dominating_information() {
        ...
        assert_eq!(
            iter.next(),
            Some((
                information3.state.signature_variables.clone(),
                vec![information3]
            ))
        );
        assert_eq!(iter.next(), None);
