...
use dypdl_heuristic_search::{
    create_policy_guided_dual_bound_apps, create_policy_guided_user_priority_apps,
    create_user_priority_apps, FEvaluatorType, Parameters, PolicyEvaluators,
...
/// Anytime Column Progressive Seach (APPS) solver.
///
/// This performs APPS using user-provided g-, h-, and f-values and a user-provided policy.
...
#[pyclass(unsendable, name = "UserPriorityAPPS")]
pub struct UserPriorityAppsPy(
    WrappedSolver<Box<dyn Search<Integer>>, Box<dyn Search<OrderedContinuous>>>,
);

#[pymethods]
impl UserPriorityAppsPy {
    #[new]
    #[pyo3(
    ...
    ) -> PyResult<UserPriorityAppsPy> {
        if !quiet {
            println!(
                "Solver: UserPriorityAPPS from DIDPPy v{}",
        ...
        if let Some(h_evaluator) = h_evaluator {
            ...
            if float_cost {
                    ...
                    (policy, policy_f_evaluator)
                {
                    ...
                    create_policy_guided_user_priority_apps(
                        model,
                        parameters,
                ...
                Ok(UserPriorityAppsPy(WrappedSolver::Float(solver)))
            } else {
                let primal_bound = if let Some(primal_bound) = primal_bound {
                ...
                Ok(UserPriorityAppsPy(WrappedSolver::Int(solver)))
            }
        } else {
            ...
            if float_cost {
                ...
                let solver = create_policy_guided_dual_bound_apps(
                    model,
                    parameters,
                    bound_evaluator_type,
                    policy_evaluators,
    ...
    /// >>> solver = dp.UserPriorityAPPS(model, quiet=True)
