    ...
    V: TransitionInterface + Clone + Default,
{
    ...
    let expansion_limit = parameters.beam_search_parameters.parameters.expansion_limit;
    ...
    while !current_beam.is_empty() {
        ...
        for node in current_beam.drain(..) {
            ...
            let time_out = time_keeper.check_time_limit(quiet);

            if time_out || expansion_limit.map_or(false, |limit| expanded >= limit) {
                if !time_out && !quiet {
