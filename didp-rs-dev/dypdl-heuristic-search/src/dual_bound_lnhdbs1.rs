...
/// It uses a parallel beam search algorithm, hash distributed beam search 2 (HDBS1), instead of sequential beam search.
...
///     BeamSearchParameters, CabsParameters, create_dual_bound_lnhdbs1, FEvaluatorType,
...
///
/// let threads = 1;
/// let mut solver = create_dual_bound_lnhdbs1(
...
/// ```
pub fn create_dual_bound_lnhdbs1<T>(
    ...
    <T as str::FromStr>::Err: fmt::Debug,
{
    ...
    if model.has_dual_bounds() {
        ...
        let solution = solution.unwrap_or_else(|| {
            ...
            let mut cabs =
                Cabs::<_, _, _, _, _, _>::new(input, beam_search.clone(), cabs_parameters);
        ...
        parameters.beam_search_parameters.parameters.expansion_limit = parameters
            .beam_search_parameters
            .parameters
            .expansion_limit
            .map(|expansion_limit| expansion_limit.saturating_sub(solution.expanded));
        ...
        Box::new(Lnbs::<_, _, _, _, _, _, _>::new(
            input,
            beam_search,
            transition_mutex,
            parameters,
