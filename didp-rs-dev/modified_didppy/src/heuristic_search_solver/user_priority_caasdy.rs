...
use dypdl_heuristic_search::{
    create_policy_guided_caasdy, create_policy_guided_user_priority_caasdy,
    create_user_priority_caasdy, FEvaluatorType, Parameters, PolicyEvaluators, Search,
    UserEvaluators,
};
...
///
/// This performs cost-algebraic A* using user-provided g-, h-, and f-values and a user-provided policy.
...
#[pyclass(unsendable, name = "UserPriorityCAASDy")]
pub struct UserPriorityCaasdyPy(
    WrappedSolver<Box<dyn Search<Integer>>, Box<dyn Search<OrderedContinuous>>>,
);

#[pymethods]
impl UserPriorityCaasdyPy {
    #[new]
    #[pyo3(
        text_signature = "(model, h_evaluator=None, g_evaluators=None, f_operator=None, policy=None, policy_f_evaluator=None, no_policy_accumulation=False, bound_operator=didppy.FOperator.Plus, primal_bound=None, time_limit=None, get_all_solutions=False, quiet=False, initial_registry_capacity=1000000)"
    )]
    #[pyo3(signature = (
        model,
        h_evaluator = None,
    ...
    ) -> PyResult<UserPriorityCaasdyPy> {
        if !quiet {
            println!(
                "Solver: UserPriorityCAASDy from DIDPPy v{}",
        ...
        if let Some(h_evaluator) = h_evaluator {
            ...
            if float_cost {
                    ...
                    (policy, policy_f_evaluator)
                {
                    ...
                    create_policy_guided_user_priority_caasdy(
                        model,
                        parameters,
                ...
                Ok(UserPriorityCaasdyPy(WrappedSolver::Float(solver)))
            } else {
                let primal_bound = if let Some(primal_bound) = primal_bound {
                ...
                Ok(UserPriorityCaasdyPy(WrappedSolver::Int(solver)))
            }
        } else {
            ...
            if float_cost {
                ...
                let solver = create_policy_guided_caasdy(
                    model,
                    parameters,
                    bound_evaluator_type,
                    policy_evaluators,
