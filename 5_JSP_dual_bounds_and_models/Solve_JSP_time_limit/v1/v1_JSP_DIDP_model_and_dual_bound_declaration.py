...
def dual_bound_expression_function(didp_bundle):
    ...
    # =========================================================
    # Bound 3 & 4: One-Machine Bounds (Preemptive / Non-Preemptive)
    # Logic: Cache the SOLVER RESULT for a specific set of (r, p, q) tasks.
    #        Inputs (r) are dynamic, so they must be part of the key.
    # =========================================================
    @lru_cache(maxsize=100000)
    def _solve_1mach_preemptive_core(tasks_tuple):
        # Reconstruct list from tuple
        # tasks entries are (r, p, q)
        tasks = [list(t) for t in tasks_tuple]

        # Tasks are already sorted by Release time (r) by the wrapper
        time_now = 0.0
        ready_queue = [] # (-q, task_idx)
        task_idx = 0
        n_tasks = len(tasks)
        max_bound = 0.0
        active_idx = -1

        while task_idx < n_tasks or ready_queue or active_idx != -1:
            if active_idx == -1 and not ready_queue and task_idx < n_tasks:
            ...
            if active_idx != -1 and ready_queue:
                current_q = tasks[active_idx][2]
                best_waiting_q = -ready_queue[0][0]
                if best_waiting_q > current_q:
                    heapq.heappush(ready_queue, (-current_q, active_idx))
                    active_idx = -1

            if active_idx == -1 and ready_queue:
                _, active_idx = heapq.heappop(ready_queue)

            if active_idx != -1:
                rem_p = tasks[active_idx][1]
                dt = min(rem_p, tasks[task_idx][0] - time_now) if task_idx < n_tasks else rem_p
                time_now += dt
                tasks[active_idx][1] -= dt

                if tasks[active_idx][1] <= 1e-9:
                    finish_q = tasks[active_idx][2]
                    max_bound = max(max_bound, time_now + finish_q)
                    active_idx = -1
    ...
    def h_1mach_preemptive(state):
        unscheduled_set = set(state[unscheduled_var])

        # 1. Dynamic Heads Calculation
        op_heads = {}
        for j in range(num_jobs):
            curr_avail = 0.0
            for op in job_ops_map[j]:
                if op in unscheduled_set:
                    op_heads[op] = curr_avail
                    curr_avail += op_processing_time[op]
                else:
                    curr_avail = state[op_c_vars[op]]
        ...
        for m in range(num_machines):
            ...
            # Sort for Canonicalization (Cache Hit) & Algorithm Requirement
            tasks.sort(key=lambda x: x[0]) 
            tasks_tuple = tuple(tasks)

            bound = _solve_1mach_preemptive_core(tasks_tuple)
            if bound > max_bound: max_bound = bound

        return max_bound

    @lru_cache(maxsize=10000)
    def _solve_1mach_non_preemptive_core(tasks_tuple):
        tasks = [list(t) for t in tasks_tuple]
        time_now = 0.0
        ready_queue = []
        task_idx = 0
        ...
        while task_idx < n_tasks or ready_queue:
            ...
            if ready_queue:
                ...
                time_now += p
                max_bound = max(max_bound, time_now + -neg_q)
        return float(max_bound)
    ...
    def h_1mach_non_preemptive(state):
        ...
        for m in range(num_machines):
            ...
            bound = _solve_1mach_non_preemptive_core(tasks_tuple)
            if bound > max_bound: max_bound = bound
        return max_bound
