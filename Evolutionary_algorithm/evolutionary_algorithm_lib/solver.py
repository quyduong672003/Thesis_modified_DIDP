...
def combining_modified_didppy_solver_with_chromosome(chromosome, 
                                                    ...
                                                    solver_quite = True):
    """
    Runs the DIDP solver.
    Prints the report to console AND returns the data tuple.
    """
    ...
    try:  
        ...
        # --- 5. Output Results (Print + Return) ---
        if solution.cost is not None:
            # Case A: Found Solution
            if output_other_result:
                # 1. PRINT to Console (Side Effect)
                if print_timing_stats:
                    print_comprehensive_solver_report(
                        cost=solution.cost, 
                ...
                return (solution.cost, solution.is_optimal, solution.generated, solution.expanded, timing_info)
            else:
                return solution.cost
        else:
            # Case B: Timeout / Infeasible
            if output_other_result:
                # Still return tuple to prevent crash
                return (float('inf'), False, solution.generated, solution.expanded, timing_info)
            else:
                return float('inf')
...
def old_combining_modified_didppy_solver_with_chromosome(chromosome, 
                                                    didp_model_registry, 
                                                    dual_bound_expression_function, 
                                                    solver_time_limit = None,
                                                    output_other_result = False,
